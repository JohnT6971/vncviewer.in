...
public class CMsgReader {
	...
	protected void readSetVMwareCursor(int width, int height, Point hotspot, final Runnable runnable) {
		...
		if(type == 0) {
			...
			java.nio.ByteBuffer andMask = java.nio.ByteBuffer.allocate(len);
			java.nio.ByteBuffer xorMask = java.nio.ByteBuffer.allocate(len);
			
			java.nio.ByteBuffer data = java.nio.ByteBuffer.allocate(width * height * 4);
			
			java.nio.ByteBuffer andIn;
			java.nio.ByteBuffer xorIn;
			...
			andIn = java.nio.ByteBuffer.wrap(andMask.array());
			xorIn = java.nio.ByteBuffer.wrap(xorMask.array());
			out = java.nio.ByteBuffer.wrap(data.array());
