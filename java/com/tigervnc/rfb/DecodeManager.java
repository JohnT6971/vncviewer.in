...
import static com.tigervnc.rfb.Decoder.DecoderFlags.DecoderOrdered;
import static com.tigervnc.rfb.Decoder.DecoderFlags.DecoderPartiallyOrdered;
...
public class DecodeManager {
	...
	public DecodeManager(CConnection conn) {
		...
		freeBuffers = new ArrayDeque <>(cpuCount * 2);
		workQueue = new ArrayDeque <>(cpuCount);
		threads = new ArrayList <>(cpuCount);
		while(cpuCount-- > 0) {
			...
			try {
				...
				threads.add(new com.tigervnc.rfb.DecodeManager.DecodeThread(this));
	...
	public void decodeRect(Rect r, int encoding,
		...
		entry = new com.tigervnc.rfb.DecodeManager.QueueEntry();
	...
	private static class QueueEntry {
		...
		public com.tigervnc.rfb.ModifiablePixelBuffer pb;
	...
	private static class DecodeThread implements Runnable {
		...
		private final DecodeManager manager;
	...
	private final CConnection conn;
	private final Decoder[] decoders;
	
	private final ArrayDeque <MemOutStream> freeBuffers;
	private final ArrayDeque <QueueEntry> workQueue;
	
	private final ReentrantLock queueMutex;
	private final Condition producerCond;
	private final Condition consumerCond;
	
	private final List <DecodeThread> threads;
